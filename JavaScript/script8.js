...
const slider1 = document.querySelector('#myRange')
const slider2 = document.querySelector('#myYear')
const slider3 = document.querySelector(`#lenght`)
const imdb = document.querySelector('#imdb')
const myYear = document.querySelector("#Year")
const lenght1 = document.querySelector(`#lenght1`)
const button1 = document.querySelector('.btnImdb')
const button2 = document.querySelector('.btnYears')
const button3 = document.querySelector(`.btnlenght`)
    ...
    arr.map(movie => {
        container.innerHTML += `
            <div class="card">
                ...
                <div class="grow1">
                    <h1>${movie.titleLtu}</h1>
                    <h2>${movie.genre}</h2>
                    <h3><i class="fab fa-imdb"></i> ${movie.imdbRating}</h3>
...
genresSelect.onchange = () => {
    ...
    if(genreValue === "visi") {
        return appendMovies(movies)
    }

    for (let i = 0; i < movies.length; i++) {
        if(movies[i].genre.includes(genreValue)) {
    ...
    const flt = movies.filter(x => x.genre.includes(genreValue))
    appendMovies(flt)
}

slider1.oninput = () => {
    imdb.innerText = "Imdb: "+slider1.value
}

button1.onclick = () => {
    const filtered = movies.filter(x => x.imdbRating >= slider1.value )
    appendMovies(filtered)
}


slider2.oninput = () => {
    myYear.innerText = "Year: "+slider2.value
}
button2.onclick = () => {
    const filtered = movies.filter(x => x.year >= slider2.value )
    appendMovies(filtered)
}
slider3.oninput = () => {
    lenght1.innerText = "Lenght: "+slider3.value
}
button3.onclick = () => {
    const filtered = movies.filter(x => x.length >= slider3.value )
    appendMovies(filtered)

}

function byDesLenght(lenght){
    const result = movies.filter(x => x.description.length >= lenght)
    console.log(result)
}
byDesLenght
